...
from tests.mocks.audio import MockPyAudio, create_mock_audio_input
...
async def test_transcribe_basic_functionality(
    ...
    with patch("agent_cli.agents.transcribe.pyperclip") as mock_pyperclip:
        ...
        with patch("agent_cli.agents.transcribe.signal_handling_context") as mock_signal_context:
            ...
            async def auto_stop():
                await asyncio.sleep(0.2)  # Short delay for transcription to start
                stop_event.set()
        ...
        assert len(copied_text) > 0
        
        # Verify console output contains transcription info
        console_output = mock_console.file.getvalue()
        assert "transcript" in console_output.lower() or "listening" in console_output.lower()
