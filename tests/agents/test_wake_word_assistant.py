...
class TestRecordAudioToBuffer:
    ...
    async def test_records_audio_to_buffer(self, mock_to_thread, mock_stream_context, mock_pyaudio, mock_logger, mock_stop_event):
    ...
    @patch("agent_cli.agents.wake_word_assistant.open_pyaudio_stream")
    @patch("agent_cli.agents.wake_word_assistant.asyncio.to_thread")
    async def test_handles_recording_error(self, mock_to_thread, mock_stream_context, mock_pyaudio, mock_logger, mock_stop_event):
    ...
    async def test_prints_recording_message_when_not_quiet(self, mock_print, mock_stream_context, mock_pyaudio, mock_logger, mock_stop_event):
        ...
        mock_print.assert_called_once_with("🎤 Recording... Say the wake word again to stop", style="green")
...
class TestAsyncMain:
    ...
    async def test_lists_input_devices_and_exits(self, mock_list_devices, mock_pyaudio_context, sample_configs):
    ...
    async def test_lists_output_devices_and_exits(self, mock_list_devices, mock_pyaudio_context, sample_configs):
    ...
    async def test_wake_word_detection_loop(
        self, mock_detect, mock_signal_context, mock_live, mock_input_device, mock_pyaudio_context, sample_configs
    ):
...
class TestWakeWordAssistantCommand:
    ...
    def test_command_list_input_devices(
        self, mock_asyncio_run, mock_pid_context, mock_stop_or_status, mock_pyaudio_context, mock_list_devices
    ):
    ...
    @patch("agent_cli.agents.wake_word_assistant.process_manager.pid_file_context")
    @patch("agent_cli.agents.wake_word_assistant.asyncio.run")
    def test_command_with_custom_parameters(self, mock_asyncio_run, mock_pid_context, mock_stop_or_status):
        ...
        result = runner.invoke(app, [
            "wake-word-assistant",
            "--wake-word", "custom_word",
            "--wake-server-port", "12345",
