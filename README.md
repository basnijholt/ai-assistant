# GitHub Downloader (ai-assistant)

`ai-assistant` is a Python command-line tool that allows you to download files from GitHub repositories, including private ones, by leveraging your existing `gh` (GitHub CLI) authentication.

> [!TIP]
> If using [`uv`](https://docs.astral.sh/uv/), just run:
>
> 1. `uvx dotbins get cli/cli --name gh` to install the GitHub CLI.
> 2. `uvx ai-assistant --help` to see the help message.
...
After installation, you can use the `ai-assistant` command:

```bash
ai-assistant REPO_OWNER REPO_NAME FILE_PATH [OPTIONS]
```

<details>
<summary>See the output of <code>ai-assistant -h</code></summary>
...
```yaml
 ...
 Usage: ai-assistant [OPTIONS] REPO_OWNER REPO_NAME FILE_PATH
...
ai-assistant octocat Spoon-Knife README.md

# Download from a specific branch
ai-assistant octocat Spoon-Knife README.md --branch main

# Save to a specific location
ai-assistant octocat Spoon-Knife README.md --output ./my_readme.md

# Download from a different branch
ai-assistant microsoft vscode package.json --branch release/1.85
```
